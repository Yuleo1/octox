...
impl Register {
    fn addr(self, base_addr: usize) -> *mut u8 {
        match self {
            Self::RBR | Self::THR => (base_addr + 0) as *mut u8,
...
impl Uart {
    ...
    fn start(&mut self) {
        loop {
            ...
            PROCS.wakeup(&self.tx_r as *const _ as usize);
            self.write(THR, c);
        }
    }
}
    ...
    pub fn putc(&self, c: u8) {
        ...
        loop {
            if uart_guard.tx_w == uart_guard.tx_r + Wrapping(UART_TX_BUF_SIZE) {
                ...
                uart_guard = p.sleep(&uart_guard.tx_r as *const _ as usize, uart_guard);
            } else {
